use sdkwork_routes_messaging_app_api::{
    messaging_app_api_manifest, MESSAGING_APP_API_AUTH_MODE, MESSAGING_APP_API_AUTHORITY,
...
fn app_api_routes_use_app_prefix_and_dual_token_auth() {
    ...
    assert!(manifest
        .routes
        .iter()
        .all(|route| !route.operation_id.contains("dialogueThread") && !route.path.contains("/im/")));
}
