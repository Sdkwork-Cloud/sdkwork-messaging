use sdkwork_routes_messaging_backend_api::{
    messaging_backend_api_manifest, MESSAGING_BACKEND_API_AUTH_MODE,
    MESSAGING_BACKEND_API_AUTHORITY, MESSAGING_BACKEND_API_PREFIX,
    MESSAGING_BACKEND_SDK_FAMILY,
};
...
fn declares_standard_backend_api_route_manifest() {
    ...
    assert_eq!(manifest.package_name, "sdkwork-routes-messaging-backend-api");
...
fn backend_api_routes_use_backend_prefix_and_dual_token_auth() {
    ...
    for route in &manifest.routes {
        ...
        assert_eq!(route.ownership_api_authority, MESSAGING_BACKEND_API_AUTHORITY);
        assert_eq!(route.source_route_crate, "sdkwork-routes-messaging-backend-api");
    }
